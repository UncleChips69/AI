...
% Recursive case: Inserting at a position other than the 
beginning
insert_nth(I, N, [H|T], [H|R]) :-
N > 1, % Ensure we're not trying to insert at a position 
less than 1
N1 is N - 1, % Decrement the position
insert_nth(I, N1, T, R). % Recursive call with the 
decremented position and the tail of the list
